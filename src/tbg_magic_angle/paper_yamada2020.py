...
_SMALL_ANGLE_CONJECTURE_ORIGINAL = r"""Define the small-angle (continuum) limit of twisted bilayer graphene as the regime where the twist angle $\alpha\to 0$ (equivalently, commensurate indices become large) and the energy gap at the moiré $K$ point due to intervalley ($K$–$K'$) coupling becomes negligibly small, so that the upper two bands near charge neutrality are nearly degenerate. It is conjectured that the four-Dirac-point merging mechanism identified at moderate twist angles in commensurate tight-binding calculations persists continuously into this small-angle limit and provides the microscopic origin of the vanishing Dirac velocity at the magic angles associated with nearly flat bands. This conjecture assumes that no intervening topological or symmetry-driven reconstruction occurs as $\alpha$ decreases, that analogous moving Dirac points and trajectories exist in the small-angle limit, and that additional physical effects such as lattice relaxation and electron-electron interactions do not qualitatively alter the mechanism."""

_ROOT_ORIGINAL = r"""In twisted bilayer graphene at sufficiently small twist angle, where the energy gap at the moiré $K$ point between the $2n_0$th and $(2n_0+1)$th bands becomes negligibly small and the upper two bands near charge neutrality become nearly degenerate, the same band-structure mechanism in which three moving Dirac points merge with the fixed Dirac point at $K$ is proposed to persist and to produce vanishing Dirac velocity at $K$. In this conjecture, the resulting suppression of the Dirac velocity at $K$ contributes to the emergence of extremely narrow (nearly flat) bands at the magic angles."""


small_angle_dirac_merging_conjecture = claim(
...
merged_dirac_magic_angle_flat_band = claim(
...
derive_merged_dirac_magic_angle_flat_band = deduction(
